...
async def auth_login(data: dict):
    ...
    try:
        ...
        await response_web_query(init_data, "Login", render_template("login_422", lang))
        return

    status, user = await service.login(init_data.user.id, valid.email, valid.password)

    if status == 200:
        await response_web_query(init_data, "Login", render_template_user("login_200", user.user))
    if status == 400:
        lang = process_language(init_data.user)
        await response_web_query(init_data, "Login", render_template("login_400", lang))
    if status == 500:
        lang = process_language(init_data.user)
        await response_web_query(init_data, "Login", render_template("auth_not_available", lang))
...
async def auth_register(data: dict):
    ...
    try:
        ...
        user = await api_service.get_by_telegram_user_id(init_data.user.id)
        lang = process_language(init_data.user, user.user)
        await response_web_query(init_data, "Login", render_template("register_422", lang))
        return

    status = await service.register(init_data.user.id, init_data.user.username, valid)
    lang = process_language(init_data.user, None)
    if status == 400:
        await response_web_query(init_data, "Sign Up", render_template("register_400", lang))
    if status == 201:
        await response_web_query(init_data, "Sign Up", render_template("register_201", lang))
    if status == 500:
        await response_web_query(init_data, "Sign Up+"
                                            "", render_template("auth_not_available", lang))
    return ORJSONResponse({"ok": True})
