...
async def pull_order_create(
    order: api_schemas.Order, categories: list[str], configs: list[str], **_kwargs
) -> models.OrderResponse:
    return await pull_create(order, categories, configs, False)


async def pull_order_update(order: api_schemas.Order, configs: list[str], **_kwargs) -> models.OrderResponse:
    return await pull_update(order, configs, False)


async def pull_order_delete(order: api_schemas.Order, **_kwargs) -> models.OrderResponse:
    return await pull_delete(order, False)


async def pull_preorder_create(
    preorder: api_schemas.PreOrder, categories: list[str], configs: list[str], **_kwargs
) -> models.OrderResponse:
    return await pull_create(preorder, categories, configs, True)


async def pull_preorder_update(preorder: api_schemas.PreOrder, configs: list[str], **_kwargs) -> models.OrderResponse:
    return await pull_update(preorder, configs, True)


async def pull_preorder_delete(preorder: api_schemas.PreOrder, **_kwargs) -> models.OrderResponse:
...
async def pull_booster_resp_yes(
    order_id: str, user: api_schemas.User, response: response_flows.models.OrderResponse, **_kwargs
...
async def pull_booster_resp_no(order_id: str, user: api_schemas.User, **_kwargs) -> list[models.MessageResponse]:
    return await response_flows.response_declined(order_id, user)


async def send_request_verify(user: api_schemas.User, token: str, **_kwargs) -> models.MessageResponse:
...
async def send_verified(user: api_schemas.User, **_kwargs) -> list[models.MessageResponse]:
    resp = []
...
async def send_order_close_request(
    user: api_schemas.User, order_id: str, url: str, message: str, **_kwargs
) -> models.MessageResponse:
...
async def send_logged_notify(user: api_schemas.User, **_kwargs) -> models.MessageResponse:
    data = {"user": user}
    _, status = await message_service.create(
        message_models.MessageCreate(
...
async def send_registered_notify(user: api_schemas.User, **_kwargs) -> models.MessageResponse:
    data = {"user": user}
    _, status = await message_service.create(
        message_models.MessageCreate(
...
async def send_order_sent_notify(order_id: str, pull_payload, **_kwargs) -> models.MessageResponse:
...
async def send_order_edited_notify(order_id: str, pull_payload, **_kwargs) -> models.MessageResponse:
    data = {"order_id": order_id, "payload": pull_payload}
    _, status = await message_service.create(
        message_models.MessageCreate(
...
async def send_order_deleted_notify(order_id: str, pull_payload, **_kwargs) -> models.MessageResponse:
    data = {"order_id": order_id, "payload": pull_payload}
    _, status = await message_service.create(
        message_models.MessageCreate(
...
async def send_response_chose_notify(
    order_id: str, responses: int, user: api_schemas.User, **_kwargs
) -> models.MessageResponse:
...
async def send_order_paid_notify(order, user, **_kwargs) -> models.MessageResponse:
