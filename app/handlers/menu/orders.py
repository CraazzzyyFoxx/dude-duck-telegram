...
async def paging_getter_aco(dialog_manager: DialogManager, **_kwargs):
    ...
    data = await api_flows.me_get_orders(d.from_user.id, status, page=1 + int(current_page / 10))
    user = dialog_manager.start_data.get("user", None)
    return {
        "user": user,
        "pages": data.total if data.total > 0 else 1,
        "current_page": current_page + 1,
        "order": data.results[current_page % data.per_page] if data.total > 0 else [],
    }
