...
async def handle_message_expired_error(event: ErrorEvent, user=None, dialog_manager=None):
    ...
    if event.update.callback_query is not None:
        ...
        await event.update.callback_query.answer(render_template("expired", lang), show_alert=True)
    if event.update.message is not None:
        lang = process_language(event.update.message.from_user, user)
        await event.update.message.answer(render_template("expired", lang))
...
async def handle_message_server_error(event: ErrorEvent, user=None, dialog_manager=None):
    ...
    if event.update.callback_query is not None:
        lang = process_language(event.update.callback_query.from_user, user)
        await event.update.callback_query.answer(render_template("internal_error", lang), show_alert=True)
    if event.update.message is not None:
        lang = process_language(event.update.message.from_user, user)
        await event.update.message.answer(render_template("internal_error", lang))
    try:
...
async def on_unknown_intent(event: ErrorEvent, dialog_manager: DialogManager, user=None):
    if event.update.callback_query:
        lang = process_language(event.update.callback_query.from_user, user)
        await event.update.callback_query.answer(render_template("menu_restarted", lang))
        try:
