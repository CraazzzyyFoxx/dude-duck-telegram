...
async def register(message: types.Message, user: models.UserDB) -> None:
    ...
    web_app = WebAppInfo(url=f"{config.app.auth_url}telegram/auth/signup?message_id={msg.message_id}")
    await msg.edit_text(
...
async def command_webview(message: Message, user: models.UserDB) -> None:
    ...
    web_app = WebAppInfo(url=f"{config.app.auth_url}telegram/auth/login?message_id={msg.message_id}")
    await msg.edit_text(
